...
describe("orchestrator-manager roster", () => {
  it("persists the three bounded orchestration roles and maps them to the built-in provider", () => {
    ...
    expect(managers.map((manager) => manager.role)).toEqual(["Temporary CEO · Orchestrator", "Delivery Orchestrator", "Quality Orchestrator"]);
    expect(getEmployeeProvider("Atlas")).toBe("manus");
    expect(getEmployeeProvider("Nova")).toBe("manus");
  });

  it("adds all active orchestrators to real discussions and prefers non-temporary orchestration for synthesis", () => {
    resetStateForTests();
    expect(selectEmployeesForTask("Build a responsive settings page")).toEqual(expect.arrayContaining(["Manus", "Atlas", "Nova", "Gemini"]));
    expect(selectSynthesisEmployee([{ employee: "Atlas" }, { employee: "Nova" }, { employee: "Manus" }])).toBe("Atlas");
  });
});
