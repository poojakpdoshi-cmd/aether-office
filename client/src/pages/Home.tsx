...
function StatusPill({ status }: { status: EmployeeStatus | "SETUP REQUIRED" }) {
  return (
...
export default function Home() {
  ...
  const configuredProviderIds = new Set(providerStatuses.filter((provider) => provider.configured && provider.verified).map((provider) => provider.id));
  ...
  const liveEmployees = (dashboard?.employees ?? []).map((profile) => {
    const known = employees.find((employee) => employee.name === profile.id);
    const setupRequired = setupRequiredEmployeeIds.has(profile.id);
    return { name: profile.id, shortName: known?.shortName ?? profile.id.split(/\s+/).map((part) => part[0]).join("").slice(0, 2), role: profile.role, focus: setupRequired ? `Setup required · add ${profile.provider} API key` : known?.focus ?? `Configured ${profile.provider} profile`, status: setupRequired ? "IDLE" as EmployeeStatus : profile.status as EmployeeStatus, displayStatus: setupRequired ? "SETUP REQUIRED" as const : profile.status as EmployeeStatus, accent: known?.accent ?? "from-emerald-300 to-teal-600" };
  });
  ...
  const renderOfficeControl = () => {
    ...
    const isEmployeeComputer = Boolean(relatedEmployee && officeFocus === `${relatedEmployee.name} Computer`);
    const providerStatus = relatedEmployee ? providerStatuses.find((provider) => provider.label === relatedEmployee.name) : undefined;
    const managers = (dashboard?.employees ?? []).filter((employee) => ["Manus", "Atlas", "Nova", "Sentinel"].includes(employee.id)).map((employee) => ({ ...employee, status: employeeDisplayStatus(employee) }));
    const activeManus = managers.find((employee) => employee.id === "Manus");
    const manuscriptDays = activeManus?.temporaryUntil ? Math.max(0, Math.ceil((activeManus.temporaryUntil - Date.now()) / 86_400_000)) : null;
    const manusLifecycle = activeManus ? `Manus is a temporary primary orchestrator. About ${manuscriptDays ?? 0} day${manuscriptDays === 1 ? "" : "s"} remain; Atlas, Nova, and Sentinel will continue when this temporary assignment ends.` : "Manus’s temporary assignment has ended. Atlas continues delivery coordination, Nova continues quality coordination, and Sentinel continues real-error oversight.";
    ...
    return <div className="office-game-world"><LiveOffice
      ...
      sideControl={<OfficeControlChatbox
        ...
        onUpload={uploadFile}
        onOpenServiceFloor={() => setShowWorldControls(true)}
      />}
    />
    <section className="office-room-trail" aria-label="Employee rooms along the office route"><div><p>SCROLL THE OFFICE WORLD</p><h2>Explore each employee room</h2><span>Every room opens only the selected employee’s real current work and authorized sandbox state.</span></div><div className="office-room-trail-grid">{liveEmployees.map((employee) => <button type="button" key={employee.name} onClick={() => setOfficeFocus(`${employee.name} Room`)}><span>{employee.shortName}</span><div><strong>{employee.name}&apos;s room</strong><small>{employee.displayStatus === "ERROR" ? employee.role : `${employee.displayStatus} · ${employee.role}`}</small></div><i>Enter</i></button>)}</div></section>
    {showWorldControls ? <OfficeWorldControls providers={providerStatuses} approvalMode={mode as "Safe Mode" | "Team Mode" | "Autonomous Mode"} provisionPending={provisionEmployeesMutation.isPending} openRouterPending={provisionOpenRouterProfilesMutation.isPending} workspaceRoot={workspaceQuery.data?.root} workspacePending={selectWorkspaceMutation.isPending} workspaceError={selectWorkspaceMutation.error?.message} onConfigureProvider={setSetupProvider} onProvision={(provider, count) => provisionEmployeesMutation.mutate({ provider, count, ownerConfirmed: true })} onProvisionOpenRouter={(model, count) => provisionOpenRouterProfilesMutation.mutate({ model, count, ownerConfirmed: true })} onSelectWorkspace={(path) => selectWorkspaceMutation.mutate({ path })} onSetApprovalMode={(nextMode) => { setMode(nextMode); approvalModeMutation.mutate({ mode: nextMode }); }} onOpenSettings={() => setActiveView("Settings")} onOpenRoster={() => setActiveView("Employees")} onClose={() => setShowWorldControls(false)} /> : null}
    {officeFocus ? <section className="mx-auto w-full max-w-[1120px] rounded-2xl border border-white/10 bg-[#0d1527]/90 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-sky-200">{officeFocus}</p>{isEmployeeRoom ? <><h2 className="mt-2 text-lg font-semibold text-white">{relatedEmployee!.name}&apos;s Room</h2><p className="mt-2 text-sm text-slate-300">{relatedEmployee!.role} · {relatedEmployee!.status}</p><p className="mt-3 max-w-2xl text-xs leading-5 text-slate-400">Enter the room first. Then tap the employee&apos;s computer inside their room to open the real isolated monitor.</p></> : isEmployeeComputer ? <><h2 className="mt-2 text-lg font-semibold text-white">{relatedEmployee!.name}&apos;s Computer</h2><p className="mt-2 text-sm text-slate-300">Live isolated monitor · {relatedEmployee!.status}</p><p className="mt-3 max-w-2xl text-xs leading-5 text-slate-400">Only current authorized container process and sandbox data appear here. Idle and unavailable states remain explicit.</p></> : <><h2 className="mt-2 text-lg font-semibold text-white">{officeFocus}</h2><p className="mt-2 text-sm text-slate-300">{officeFocus === "DeepDiscuss Room" ? "Current meeting activity appears only after a real provider-backed discussion starts." : "Select a worker cabin or computer to inspect the real employee runtime."}</p></>}</div><Button type="button" size="sm" variant="outline" onClick={() => setOfficeFocus(null)} className="border-white/10 bg-white/[0.03] text-slate-200">Back to office</Button></div>{relatedEmployee ? <>{isEmployeeRoom ? <EmployeeRoomScene employee={relatedEmployee} onOpenComputer={() => setOfficeFocus(`${relatedEmployee.name} Computer`)} /> : null}{isEmployeeComputer ? <><EmployeeComputerMonitor employee={relatedEmployee.name} /><EmployeeInspectionPanel employee={relatedEmployee} providerLabel={providerStatus?.label ?? "Unconfigured"} providerModel={providerStatus?.model} snapshot={employeeInspectionQuery.data} gitDiff={gitDiffQuery.data} /></> : null}</> : null}{officeFocus === "DeepDiscuss Room" && latestMeeting ? <MeetingCollaborationPulse meeting={latestMeeting} /> : null}<VerifiedActivityTimeline activities={dashboard?.activities ?? []} /></section> : null}</div>;
  };
              ...
              <div key={provider} className="flex items-center justify-between rounded-xl border border-white/[0.08] bg-white/[0.025] p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={cn("mt-0.5 text-xs", status?.availability === "retired-gated" ? "text-amber-300" : status?.verified ? "text-emerald-300" : status?.configured ? "text-amber-300" : "text-slate-500")}>{status?.availability === "retired-gated" ? "Retired compatibility route" : status?.verified ? "Verified" : status?.configured ? "Re-verify key" : "Not configured"}</p>
