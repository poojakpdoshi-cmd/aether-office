...
function EmployeeInspectionPanel({ employee, providerLabel, providerModel, snapshot, gitDiff }: { employee: Employee; providerLabel: string; providerModel?: string; snapshot?: EmployeeInspection; gitDiff?: string }) {
  ...
  return (
    <section className="mt-5 rounded-xl border border-sky-300/15 bg-slate-950/55 p-4">
      ...
      <p className="mt-3 rounded-lg border border-white/[0.06] bg-white/[0.025] px-3 py-2 text-xs leading-5 text-slate-300">{snapshot?.safeTaskSummary ?? "Loading recorded employee workspace activity…"}</p>
...
function ProjectPreviewPanel({ preview, draftUrl, onDraftUrlChange, onConfigure, saving, browserEvidence: inputBrowserEvidence, browserTesting = browserTestBridge.testing, canRunBrowserTest = browserTestBridge.canRun, onRunBrowserTest = browserTestBridge.run }: { preview?: ProjectPreview; draftUrl: string; onDraftUrlChange: (value: string) => void; onConfigure: () => void; saving: boolean; browserEvidence?: BrowserEvidence | null; browserTesting?: boolean; canRunBrowserTest?: boolean; onRunBrowserTest?: () => void }) {
  ...
  return <section className="mt-5 rounded-xl border border-sky-300/15 bg-slate-950/55 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-sky-200">Local project browser</p><h3 className="mt-1 text-sm font-semibold text-white">Guarded loopback preview</h3><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-400">AetherOffice accepts only explicit local HTTP URLs on localhost, 127.0.0.1, or [::1]. No remote site can be embedded or opened from this panel.</p></div><Badge className="border-white/10 bg-white/[0.06] text-[10px] text-slate-200 hover:bg-white/[0.06]">{preview?.source ?? "unavailable"}</Badge></div><div className="mt-4 flex flex-wrap gap-2"><Input value={draftUrl} onChange={(event) => onDraftUrlChange(event.target.value)} placeholder="http://localhost:5173" className="max-w-md border-white/10 bg-black/20 font-mono text-slate-100" /><Button size="sm" disabled={!draftUrl.trim() || saving} onClick={onConfigure} className="bg-sky-300 text-slate-950 hover:bg-sky-200">{saving ? "Checking…" : "Open local preview"}</Button></div>{preview?.url ? <><div className="mt-4 overflow-hidden rounded-lg border border-white/[0.08] bg-black"><div className="flex items-center justify-between gap-3 border-b border-white/[0.08] px-3 py-2"><span className="truncate font-mono text-[10px] text-slate-400">{preview.url}</span><div className="flex shrink-0 items-center gap-3"><Button size="sm" variant="outline" onClick={() => setPreviewReloadKey((key) => key + 1)} className="h-6 border-white/10 bg-white/[0.03] px-2 text-[10px] text-sky-200">Reload preview</Button><a href={preview.url} target="_blank" rel="noreferrer" className="text-[10px] font-semibold uppercase tracking-[0.1em] text-sky-200 hover:text-sky-100">Open separately</a></div></div><iframe key={previewReloadKey} title="Selected local project preview" src={preview.url} sandbox="allow-forms allow-scripts allow-same-origin" className="h-[360px] w-full bg-white" /></div></> : <p className="mt-4 rounded-lg border border-dashed border-white/[0.08] px-3 py-4 text-xs leading-5 text-slate-500">No verified local preview is configured. Start your project locally, then provide its loopback URL here. Controlled development-server output can also supply a detected local URL.</p>}{evidence ? <div className="mt-4 rounded-lg border border-white/[0.08] bg-black/20 p-3"><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-sky-200">Latest controlled evidence</p><p className="mt-1 font-mono text-[11px] text-emerald-200">$ {evidence.command} {evidence.args.join(" ")} · {evidence.status}</p><pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap font-mono text-[10px] leading-5 text-slate-400">{(evidence.stdout || evidence.stderr || "No captured output.").slice(0, 2_500)}</pre></div> : null}<div className="mt-4 rounded-lg border border-violet-300/15 bg-violet-300/[0.04] p-3"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-violet-200">Controlled browser test</p><p className="mt-1 text-xs text-slate-400">Runs local Chromium against the configured loopback preview and captures only real browser evidence.</p></div><Button size="sm" disabled={!canRunBrowserTest || browserTesting} onClick={onRunBrowserTest} className="bg-violet-300 text-slate-950 hover:bg-violet-200">{browserTesting ? "Testing…" : "Run browser test"}</Button></div>{!canRunBrowserTest ? <p className="mt-2 text-[10px] leading-4 text-amber-200">Approve a TEAM PROPOSAL, confirm the action, and configure a loopback preview before testing.</p> : null}{browserEvidence ? <div className="mt-3 grid gap-3 lg:grid-cols-2"><div className="rounded border border-white/[0.07] bg-black/20 p-2.5 text-xs text-slate-400"><p className={browserEvidence.passed ? "font-semibold text-emerald-200" : "font-semibold text-rose-200"}>{browserEvidence.passed ? "PASSED" : "FAILED"} · HTTP {browserEvidence.httpStatus ?? "—"}</p><p className="mt-1 truncate font-mono text-[10px]">{browserEvidence.finalUrl ?? browserEvidence.targetUrl}</p><p className="mt-1 text-[10px] text-slate-600">{browserEvidence.title ?? "No document title"}</p>{browserEvidence.localScreenshotPath ? <p className="mt-2 font-mono text-[10px] text-violet-200">Screenshot saved locally: {browserEvidence.localScreenshotPath}</p> : null}</div><div className="rounded border border-white/[0.07] bg-black/20 p-2.5"><p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-sky-200">Console and errors</p><pre className="mt-2 max-h-32 overflow-auto whitespace-pre-wrap font-mono text-[10px] leading-5 text-slate-400">{[...browserEvidence.console.map((entry) => `[${entry.level}] ${entry.text}`), ...browserEvidence.errors.map((error) => `[error] ${error}`)].join("\n") || "No console messages or page errors were captured."}</pre></div><div className="rounded border border-white/[0.07] bg-black/20 p-2.5 lg:col-span-2"><p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-sky-200">Network evidence</p><pre className="mt-2 max-h-32 overflow-auto whitespace-pre-wrap font-mono text-[10px] leading-5 text-slate-400">{browserEvidence.network.map((entry) => `${entry.status} ${entry.method} ${entry.url}`).join("\n") || "No qualifying network responses were captured."}</pre></div></div> : <p className="mt-3 text-xs text-slate-500">No controlled browser test has been recorded for this workspace yet.</p>}</div><p className="mt-3 text-[10px] leading-4 text-slate-600">Preview URLs and captured command output are locally scoped. Sensitive values in execution output are redacted before display.</p></section>;
}
...
export default function Home() {
  ...
  const inspectedEmployeeName = useMemo(() => {
    const deskOrLaptop = officeFocus?.replace(/ (Desk|Laptop)$/, "");
    const target = officeFocus === "Manager" ? "Manus" : deskOrLaptop;
